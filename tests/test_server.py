...
def test_list_directories(tmp_path):
    ...
    client = TestClient(app)
    resp = client.get("/api/directories", params={"path": str(tmp_path)})
    assert resp.status_code == 200
    data = resp.json()
...
def test_list_directories_hides_dotfiles(tmp_path):
...
def test_list_directories_default_path():
...
def test_list_directories_not_found():
    ...
    client = TestClient(app)
    resp = client.get("/api/directories", params={"path": "/nonexistent/path/xyz"})
    assert resp.status_code == 404
...
def test_init_project(tmp_path):
    """POST /api/projects/init creates .cody/ directory"""
    client = TestClient(app)
    resp = client.post("/api/projects/init", json={"workdir": str(tmp_path)})
    assert resp.status_code == 200
    data = resp.json()
    assert data["status"] == "success"
...
def test_init_project_idempotent(tmp_path):
    """POST /api/projects/init is safe to call twice"""
    client = TestClient(app)
    # First call
    resp = client.post("/api/projects/init", json={"workdir": str(tmp_path)})
    assert resp.status_code == 200
...
def test_init_project_preserves_existing_config(tmp_path):
    """POST /api/projects/init does not overwrite existing config"""
    cody_dir = tmp_path / ".cody"
    cody_dir.mkdir()
    (cody_dir / "config.json").write_text('{"model": "custom"}')

    client = TestClient(app)
    resp = client.post("/api/projects/init", json={"workdir": str(tmp_path)})
    assert resp.status_code == 200
    # Original config preserved
    assert '"custom"' in (cody_dir / "config.json").read_text()


def test_init_project_missing_workdir():
    """POST /api/projects/init without workdir fails validation"""
    client = TestClient(app)
    resp = client.post("/api/projects/init", json={})
    assert resp.status_code == 422


def test_init_project_nonexistent_dir():
    """POST /api/projects/init with nonexistent dir returns 404"""
    client = TestClient(app)
    resp = client.post("/api/projects/init", json={"workdir": "/nonexistent/path/xyz"})
    assert resp.status_code == 404
