...
上次审查（v1 报告）发现的 17 个问题中，**14 个已修复/改进，1 个部分改进，2 个仍需关注**：
...
| P2-12 | grep 性能限制 | ✅ **已改进** | 增加 `_MAX_FILE_SIZE` 跳过大文件，跳过时在结果末尾追加 `[NOTE]` 提示被跳过的文件名和大小，仍为纯 Python 实现 |
...
**位置**: `cody/core/tools/search.py:23-112`

已增加 `_MAX_FILE_SIZE` 限制（1MB），跳过大文件时会在结果末尾追加 `[NOTE]` 提示被跳过的文件名和大小（避免搜索结果静默不完整）。但核心仍是逐文件逐行扫描，在有 10,000+ 文件的大型项目中性能可能是瓶颈。
