"""Workflow executor for Cody runtime graphs."""
...
NodeHandler = Callable[[WorkflowState, WorkflowNode], dict[str, Any] | None]
ConditionHandler = Callable[[WorkflowState, WorkflowEdge], bool]
...
class WorkflowExecutor:
    """Execute a compiled workflow while recording trace events and checkpoints."""
    ...
    def run(
        ...
        *,
        ...
        return self._run_from_state(workflow, state, max_steps=max_steps)
        ...
        *,
        ...
        try:
            while state.current_node_id is not None:
                ...
                state = self._execute_node(workflow, state, node)
            self._record(
    ...
    def _execute_node(
        ...
        try:
            ...
            output = handler(state, node) if handler else None
            ...
            next_edge = self._select_edge(workflow, edge_state, node)
    ...
    def _select_edge(
        ...
        for edge in conditional:
            ...
            if handler is None:
                raise WorkflowExecutionError(f"No condition handler registered: {edge.condition}")
...
def _state_from_checkpoint(workflow: CompiledWorkflow, checkpoint: CheckpointRecord) -> WorkflowState:
    if not checkpoint.workflow_state:
        raise WorkflowExecutionError(f"Checkpoint has no workflow state: {checkpoint.checkpoint_id}")
